...
`build.sh` asserts the concretized lock actually matches the requested variant
(grepping `spack.lock`), so a mis-resolved external or a leaking `PrgEnv` can never
...
- **Cheap (login node):** `STOP_AFTER_CONCRETIZE=1 LFRIC_STACK=cray bash scripts/build.sh`
  ...
  is single-process and fine on the login node.
